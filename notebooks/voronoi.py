"""Voronoi tessellation plotting for spatial niche visualisation.

Vendored from ``gml-teaching-2026/003-downstream-analysis/voronoi.py`` and used by the
niche analysis Tutorial. Public API is unchanged (``voronoi_finite_polygons_2d``,
``plot_voronoi``, ``draw_voronoi_scatter``) so the lineage stays legible; see ADR-0001.

Three fixes were applied when vendoring:
  1. ``vor.points.ptp()`` -> ``np.ptp(vor.points, axis=0)`` -- ``ndarray.ptp`` was removed
     in NumPy 2.0 and Colab ships 2.0.2, so the original raises AttributeError.
  2. ``plot_voronoi`` no longer mutates the caller's ``points`` array in place.
  3. Non-Polygon results from clipping against the convex hull are skipped instead of
     raising on ``.boundary.coords``.

``voronoi_finite_polygons_2d`` is adapted from a StackOverflow answer, attributed inline.
That function is CC BY-SA 3.0 (https://creativecommons.org/licenses/by-sa/3.0/), NOT MIT --
see the Exceptions section of this repository's LICENSE. The rest of this file is MIT.
"""
import numpy as np
...
def voronoi_finite_polygons_2d(vor, radius=None):
    ...
    if radius is None:
        radius = np.ptp(vor.points, axis=0).max()   # ndarray.ptp removed in NumPy 2.0
...
# colors = [sns.color_palette('bright')[i] for i in spot_samp['neighborhood10']]
    ...
    points = np.asarray(points, dtype=float).copy()   # never mutate the caller's array
    if invert_y:
    ...
    for i,(region,alph) in enumerate(zip(regions,alpha)):
        ...
        if p.is_empty or p.geom_type != 'Polygon':
            # clipping against the convex hull can yield MultiPolygon / GeometryCollection,
            # whose .boundary.coords raises -- skip those cells rather than crash
            continue
        if p.area <size_max:
